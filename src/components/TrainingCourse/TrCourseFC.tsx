...
const TrCourse: React.FC = () => {
  ...
  async function deleteEmptyLines() {
    ...
    setTrainingLines(prevState => prevState.filter(state => state.exerciseId._id !== '6384a9c95cc12ea42d040af2') )
  ...
  async function handleSelect(e: React.ChangeEvent<HTMLSelectElement>, trainingLine: TrainingLine) {
    ...
    setTrainingLines(prevState => (prevState.splice(spliceIndex, 1, trainingLine), [...prevState]))
  }

  function handleLineChange(e: React.ChangeEvent<HTMLInputElement>, trainingLineId: string) {
    let {name, value} = e.target

    let trainingLinesData = trainingLines
    let trainingLine: any = trainingLines.find(
      (trainingLine) =>{return trainingLine._id === trainingLineId}
    );
    ...
    setTrainingLines(prevstate => (prevstate.splice(index, 1, trainingLine), [...prevstate]))
  }
